...
var defaultConst	= require('./config/default.js');		// Constants
var dbStructs		= require('./db/dbStruct.js');			// Database structures
var validation		= require('./classes/validation.js');	// String validation
...
MongoClient.connect(config.dbUrl, function(err, db){
	...
	app.post('/auth', function(req, res){
		...
		if (!validation.validateUsername(login.username)) {
		...
		} else if (!validation.validatePassword(login.password)){
				...
				if (!usrResult){
					...
					db.collection(defaultConst.colUsers).findOne({  email: tempUser.email  }, function(err, emailResult){
						...
						if (!emailResult){
							...
							// Generate salt
							bcrypt.genSalt(config.saltRounds, function(err, tempsalt) {
								if (err) throw err;
							    ...
							    bcrypt.hash(tempUser.password, tempsalt, function(err, temphash) {
							    	...
							    	db.collection(defaultConst.colUsers).insertOne(tempUser, function(err, result){
							    		...
							    		// UserData template
							    		tempUserData = dbStructs.userDataStruct();
							    		tempUserData.linkId = userId;
							    		tempUserData.ipreg = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
							    		tempUserData.language = locale.;
	...
	console.log("[Cold Start] *Ignition* *Beep* *Beep* *Vroom* ~ localhost:" + defaultConst.port + "/api");
